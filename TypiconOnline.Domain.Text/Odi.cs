...
namespace TypiconOnline.Domain.Rules.Days
{
    ...
    public class Odi : DayElementBase
    {
        ...
        /// Глас
        /// </summary>
        [XmlAttribute(XmlConstants.OdiNumberAttrName)]
        public int Number { get; set; }

        /// <summary>
        /// Тропари песни канона
        /// </summary>
        [XmlElement(XmlConstants.OdiTroparionName)]
        ...
        protected override void Validate()
        {
            if ((Number < 1) || (Number > 9))
            {
                ...
                AddBrokenConstraint(OdiBusinessConstraint.InvalidNumber, XmlConstants.KanonasOdiNode);
            }

            //тропари должны быть
            if (Troparia.Count == 0)
            {
                AddBrokenConstraint(OdiBusinessConstraint.TroparionRequired, XmlConstants.KanonasOdiNode);
            }
            else
            {
                foreach (Ymnos trop in Troparia)
                {
                    if (!trop.IsValid)
                    {
                        AppendAllBrokenConstraints(trop, XmlConstants.OdiTroparionName);
                    }
                }
            }
