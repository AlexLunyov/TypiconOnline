...
namespace TypiconOnline.Domain.Rules.Days
{
    ...
    public class OdiTroparion: ItemText
    {
        ...
        {
            //kind
            XmlAttribute kindAttr = node.Attributes[XmlConstants.OdiTroparionKindAttr];

            if (kindAttr != null)
            {
                switch (kindAttr.Value)
                {
                    case XmlConstants.TheotokionKindAttrValue:
                        Kind = OdiTroparionKind.Theotokion;
                        break;
                    case XmlConstants.TriadikoKindAttrValue:
